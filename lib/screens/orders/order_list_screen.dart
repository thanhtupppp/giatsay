...
// Extension to help with hover effects if needed, 
// though DataRow2 handles basic hover.
...
class _OrderListScreenState extends State<OrderListScreen> {
  ...
  Future<void> _printBarcode(int orderId) async {
    try {
      ...
      // Print
      await PrintService.instance.printBarcodeLabel(order, customer, 'LAUNDRY MANAGEMENT');
      
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Đã gửi in mã vạch'),
          ...
          Container(
            ...
            child: Row(
              children: [
                            ...
                            items: [
                              ...
                              ...AppConstants.orderStatuses.map((status) {
                                return DropdownMenuItem(
                                  value: status,
                                  child: Text(AppConstants.orderStatusLabels[status] ?? status),
                                );
                              }),
                            ],
                            onChanged: (value) {
                ...
                // Search with specific placeholder
                Expanded(
                  child: TextField(
                    decoration: const InputDecoration(
                      labelText: 'Tìm kiếm',
                      hintText: 'Nhập mã đơn, tên khách, SĐT...',
          ...
          Expanded(
            child: _isLoading
                ...
                : _filteredOrders.isEmpty
                    ...
                    : DataTable2(
                        columnSpacing: 24, // Tăng khoảng cách cột
                        horizontalMargin: 24,
                        minWidth: 1000,
                        dataRowHeight: 64, // Tăng chiều cao dòng
                        columns: const [
                          DataColumn2(
                          ...
                          DataColumn2(
                            label: Text('NGÀY NHẬN', style: TextStyle(fontWeight: FontWeight.bold)),
                            size: ColumnSize.M,
                          ),
                          DataColumn2(
                            label: Text('NGÀY GIAO', style: TextStyle(fontWeight: FontWeight.bold)),
                            size: ColumnSize.M,
                          ),
                          DataColumn2(
                            label: Text('TỔNG TIỀN', style: TextStyle(fontWeight: FontWeight.bold)),
                            size: ColumnSize.M,
                            numeric: true, // Canh phải số tiền
                          ),
                          DataColumn2(
                        ...
                        rows: _filteredOrders.map((order) {
                          ...
                          return DataRow2(
                            ...
                            cells: [
                              ...
                              DataCell(Text(order['customer_name'] as String)),
                              DataCell(Text(order['customer_phone'] as String)),
                              DataCell(
                                Text(
                              ...
                              DataCell(
                                Text(
                                  order['delivery_date'] != null
                                      ? DateFormat('dd/MM/yyyy').format(
                                          DateTime.parse(order['delivery_date'] as String),
                                        )
                                      : '-',
                                ),
                              ),
                                  ...
                                  children: [
                                    ...
                                    Tooltip(
                                      ...
                                      child: IconButton(
                                        ...
                                        iconSize: 20,
                                        onPressed: () => _printBarcode(order['id'] as int),
                                      ),
                                    ),
                                    
                                    // Details button replacement (Action)
                                    TextButton.icon(
              ...
              children: [
                ...
                Text(
                  NumberFormat.currency(locale: 'vi', symbol: 'đ').format(
                    _filteredOrders.fold<double>(0, (sum, order) => sum + (order['total_amount'] as num).toDouble())
                  ),
