...
class _TimesheetScreenState extends State<TimesheetScreen>
  ...
  void initState() {
    super.initState();
    final isAdmin = AuthService.instance.hasPermission(
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // 3. Get History
      final history = await _shiftRepo.getTimesheets(
  ...
  Future<void> _checkIn() async {
    ...
    final confirm = await showAppConfirmDialog(
      ...
      content:
          'Bắt đầu ca làm việc ${_selectedShift!.name} lúc ${DateFormat('HH:mm').format(DateTime.now())}?',
  ...
  Future<void> _checkOut() async {
    final confirm = await showAppConfirmDialog(
      ...
      content:
          'Kết thúc ca làm việc lúc ${DateFormat('HH:mm').format(DateTime.now())}?',
          ...
          child: Container(
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    shift == null ? 'Thêm ca làm việc' : 'Sửa ca làm việc',
                  ...
                  AppTextField(
                    ...
                    label: 'Tên ca *',
                    validator: (v) =>
                        v == null || v.isEmpty ? 'Vui lòng nhập tên ca' : null,
                  ),
    ...
    if (result == true) {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Đã lưu ca làm việc'),
              backgroundColor: AppTheme.successColor,
            ),
          );
        }
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 16),
                      Text(
                        'Đang làm việc: ${_todayTimesheet!.shiftName ?? "Ca tự do"}',
                      ...
                      Text(
                        'Check-in lúc: ${DateFormat('HH:mm').format(_todayTimesheet!.checkIn!)}',
                ...
                cells: [
                  DataCell(
                    Text(
                      DateFormat(
                        'dd/MM/yyyy',
                      ).format(DateTime.parse(t.workDate)),
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            Text('Cấu hình Ca làm việc', style: AppTheme.heading2),
            PrimaryButton(
              icon: Icons.add,
              label: 'Thêm ca mới',
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    s.isActive ? 'Đang kích hoạt' : 'Đã ẩn',
                                    style: TextStyle(
                                      color: s.isActive
                                          ? Colors.green
                                          : Colors.grey,
        ...
        const SizedBox(height: 32),
        Text('Lịch sử chấm công toàn bộ nhân viên', style: AppTheme.heading2),
        const SizedBox(height: 16),
